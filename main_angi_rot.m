function main_angi_rot(cone_type)
    % close all
    % clear
    % clc
    include_path();
    ...
    outpath     = "/home/fs0/qijia/scratch/exp_data/trajectorysimulation/recon_result/exp_22-3-22/"
    phantoms    = ["phantom_diff", "radial_dn"]

    cone_types  = {'Johnson', 'Gurney', 'improvedGurney', 'VDGurney', 'radial', 'modified_johnson','gurney_johnson','johnson_gurney','cone9'};
    % cone_type   = 6;
    disp(char(cone_types(cone_type)))
    ...
    saveas(densfig, outpath + num2str(cone_type) + "_densfig.png");
    %% ------------------------------------------------
    tic
    ...
    for fname = phantoms
        ...
        save_avw(abs(recon_img), outpath + num2str(cone_type) + "_" + fname + ".nii.gz", 'd', [res, res, res]);
    end
    ...
    save_avw(abs(PSF), outpath + num2str(cone_type) + "_PSF.nii.gz", 'd', [res, res, res]./2);
    ...
    save(outpath + num2str(cone_type) + "_metrics.mat", 'SNReff', 'fwhm', 'sidelobe_level', 'SNRreplica', 'SNRaliasing');
end
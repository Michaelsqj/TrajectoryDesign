...
str = sprintf('gencone([%g %g],[%g %g],%g,[%g %g],%g,%g,%g,%g,%g,%g,%g,[%g %g]);',RES(1),RES(2),FOV(1),FOV(2),NINT,min(THETA),max(THETA),MAXLEN,TS,SMAX,GMAX,OVERSAMPLE,DCF,MINDENS,APOD(1),APOD(2));
%disp(str);

% Make sure theta is between 1e-5 and PI/2-1e-5 
% (the slight offsets mean we don't need to worry about divide by zero errors everywhere)
...
% Determine the amount by which the waveforms will be scaled to cover the range of thetas
...
truemaxtheta = angle(cos(maxtheta)*kmax_xy+i*sin(maxtheta)*kmax_z);
truemintheta = angle(cos(mintheta)*kmax_xy+i*sin(mintheta)*kmax_z);

FOVrad_maxtheta = 1/intlineellipse(1/FOV(1),1/FOV(2),truemaxtheta);
FOVrad_mintheta = 1/intlineellipse(1/FOV(1),1/FOV(2),truemintheta);
...
str = sprintf('wcc(%g,[%g %g],%g,%g,%g,%g,%g,[%g %g],[%g %g],%g,[%g %g]);\n',pi/2-THETArange,FOVcirc,FOVrad,DCF,KMAXrange,NINT,MAXLEN*OVERSAMPLE,TS,SMAX*TS*xyscale, SMAX*TS*zscale,GMAX*xyscale,GMAX*zscale,MINDENS*densadjust,APOD(1),APOD(2));
...
% Scale the waveforms up so that they can just be multiplied by the simple scaling factor later
if (OVERSAMPLE>1)

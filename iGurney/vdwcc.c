...
double *rhofn;  // global variable as function of rho against |kr|, range normalized 0 ~ pi
int len_rhofn;

double vd_weight_fn(double krai, double kmax) {
    double rho = 1;
    int idx;
    idx = (floor(krai / (kmax/len_rhofn)) < (len_rhofn - 1)) ? floor(krai / (kmax/len_rhofn)) : (len_rhofn - 1);
    rho = rhofn[idx];
    //    rho = 1 + krai / kmax;
    return rho;
}
...
int circcirc(double d, double e, double k, double s, double type, double *x, double *y) {
    ...
    if ((d == 0) && (e == 0)) {
        ...
        *y = 0;
        ...
        c = d * d * d * d + k * k * k * k - 2 * e * e * s * s + 2 * d * d * e * e - 2 * d * d * s * s + s * s * s * s -
            2 * k * k * s * s - 2 * k * k * d * d + e * e * e * e + 2 * k * k * e * e;
...
double calcn(double k, double sintheta, double FOV1, double FOV2, double NINT, double kmax, double dcf, int nosmooth) {
    double Gtwist;
    double n;
    double c1, c2, kstart, ff, a, nt1, nt2, cc, rho;
    if (dcf < 0) {
        Gtwist = 2 * PI * sintheta * k * FOV2 / NINT;
        n = 2 * PI * FOV2 / NINT;
    } else if (dcf > 0.5) {
        rho = vd_weight_fn(k, kmax);
        printf(" %f ", rho);
        Gtwist = pow(2 * PI * sintheta * k * FOV2 / NINT * k / kmax * rho, 2) - FOV2 * FOV2 / (FOV1 * FOV1);
...
double calcdn(double k, double sintheta, double FOV1, double FOV2, double NINT, double kmax, double dcf, int nosmooth) {
    double Gtwist;
    double dn;
    double c1, c2, kstart, ff, dff, a, nt1, nt2, nt3, cc, rho;
    if (dcf < 0) {
        dn = 0;
    } else if (dcf > 0.5) {
        rho = vd_weight_fn(k, kmax);
        Gtwist = pow(2 * PI * sintheta * k * FOV2 / NINT * k / kmax * rho, 2) - pow(FOV2 / FOV1, 2);
        Gtwist = sqrt((Gtwist > 0) ? Gtwist : 0);
        dn = 8 * k * k * sintheta * PI * PI * FOV2 * FOV2 / (Gtwist * NINT * NINT * kmax * kmax) -
             Gtwist / (k * k * sintheta);
    } else if (dcf > 0) {
        c1 = 4 * PI * PI * sintheta * sintheta * FOV2 * FOV2 / (NINT * NINT);
        c2 = (FOV2 * FOV2) / (FOV1 * FOV1);
    ...
    if (g != 0) {
        ...
        if ((tc_leftover + tba / Ts) < 1) {
            ...
            for (ai = floor(tb / Ts / 2 + tc / Ts) + 1; ai <= floor(tb / Ts / 2 + tc / Ts + tba / Ts); ai++) {
                k[ai] = k[0] - Gc * tb / 2 - tb * tb / 8 * Smax / Ts - (tc * Gmax) -
                        (ai * Ts - tb / 2 - tc) * (Gc + tb / 2 * Smax / Ts) +
                        (ai * Ts - tb / 2 - tc) * (ai * Ts - tb / 2 - tc) / 2 * Smax / Ts;
            }
            k[(int) ceil(tb / Ts / 2 + tc / Ts + tba / Ts)] = 0;
        }
...
int rtz(double gcx, double gcy, double gcz, double kcx, double kcy, double kcz, double theta, int maxai, double Gmax_xy,
        double Gmax_z, double Smax_xy, double Smax_z, double Ts, double *pgx, double *pgy, double *pgz, double *pkx,
        double *pky, double *pkz, double FOV1, double FOV2, double NINT, double kmax, double dcf) {
    ...
    while ((done == 0) && (fail == 0) && (ai < (maxai))) {
        ...
        ttot = rto(sqrt(gxy * gxy + pgz[ai + 1] * pgz[ai + 1]), sqrt(kxy * kxy + pkz[ai + 1] * pkz[ai + 1]) * 4258 * Ts,
                   Smax_c, Gmax_c, 4258, Ts, 0, 0);
...
double apod(double k, double kmax, double falloff, double alpha) {
    double tt;
    tt = (0.5 + 0.5 * cos(PI * max(0, 1 / falloff * (k / kmax - 1) + 1))) *
         exp(-(alpha * alpha * k * k / (kmax * kmax)));
    return max(1e-10, tt);
}
        ...
        *pai = ai;
        ...
        while ((ai < (numpt - 1)) && (pkr[ai] < kmax)) {
            ...
            if (dorungekutta) {
                ...
                q2 = Ts * (dk + r1 / 2.0);
                r2 = Ts * fd2k(pkr[ai] + q1 / 2.0, dk + r1 / 2.0,
                               calcn(pkr[ai] + q1 / 2.0, sintheta, FOV1, FOV2, NINT, kmax, dcf, 0),
                               calcdn(pkr[ai] + q1 / 2.0, sintheta, FOV1, FOV2, NINT, kmax, dcf, 0), SMS, cmp);

                q3 = Ts * (dk + r2 / 2.0);
                r3 = Ts * fd2k(pkr[ai] + q2 / 2.0, dk + r2 / 2.0,
                               calcn(pkr[ai] + q2 / 2.0, sintheta, FOV1, FOV2, NINT, kmax, dcf, 0),
            ...
            if (dens_min > 0) {
                ggmax = NINT * sqrt((n * n * pkr[ai] * pkr[ai] * sintheta * sintheta) + 1) /
                        (2 * PI * pkr[ai] * pkr[ai] * sintheta * dens_min * apod(pkr[ai], kmax, falloff, alpha));
...
/*      pcsndk[ai+1] = pcsndk[ai]+dk*n;*/
        ...
        while ((rtzlen != -1) && (rangehigh < maxrangehigh)) {
            ...
            ai = rangehigh;
            gxy_in = sqrt(pgx[ai + 1] * pgx[ai + 1] + pgy[ai + 1] * pgy[ai + 1]);
            rtzlen = rtz(-pgx[ai + 1], -pgy[ai + 1], -pgz[ai + 1], pkx[ai], pky[ai], pkz[ai], theta, numpt, Gmax_xy,
                         Gmax_z, Smax_xy, Smax_z, Ts, prgx, prgy, prgz, prkx, prky, prkz, FOV1, FOV2, NINT, kmax, dcf);
        }
        if ((rangehigh == dlen - 2) && (rtzlen > 0)) {
            ...
            for (ci = 0; ci < 10; ci++) {
                ...
                n_cur = atan2(kyt, kxt) /
                        (4258 * Ts * max(1e-8, (sqrt(kxt * kxt + kyt * kyt + kzt * kzt) - kmaxg / sin(theta))));
                if (n_cur > n_kmax) {
                    phi_high = dphimax;
                    phi_low = 0;
                    for (bi = 0; bi < 20; bi++) {
                    ...
                    rtzlen = rtz(-gxt, -gyt, -gzt, kmaxg * 4258 * Ts, 0 * 4258 * Ts, kmaxg / tan(theta) * 4258 * Ts,
                                 theta, numpt, Gmax_xy, Gmax_z, Smax_xy, Smax_z, Ts, prgx, prgy, prgz, prkx, prky, prkz,
                                 FOV1, FOV2, NINT, kmax, dcf);
                } else {
                    rtzlen = -1;
                }
            ...
            if (Gc_bot > 0) {
                rtzlen = rtz(rgxt, rgyt, rgzt, kmaxg * 4258 * Ts, 0 * 4258 * Ts, kmaxg / tan(theta) * 4258 * Ts, theta,
                             numpt, Gmax_xy, Gmax_z, Smax_xy, Smax_z, Ts, prgx, prgy, prgz, prkx, prky, prkz, FOV1,
                             FOV2, NINT, kmax, dcf);
            } else {
                rtzlen = -1;
            }
            ...
            while ((rangehigh - rangelow) > 0) {
                ...
                rtzlen = rtz(-pgx[ai + 1], -pgy[ai + 1], -pgz[ai + 1], pkx[ai], pky[ai], pkz[ai], theta, numpt, Gmax_xy,
                             Gmax_z, Smax_xy, Smax_z, Ts, prgx, prgy, prgz, prkx, prky, prkz, FOV1, FOV2, NINT, kmax,
                             dcf);
...
mexFunction(int nlhs, mxArray *plhs[],
    ...
    if (nrhs < 13) {
        len_rhofn = 1000;
        rhofn = (double*)malloc(sizeof(double) * len_rhofn);
        for (int i = 0; i < len_rhofn; i++) {
            rhofn[i] = 1;
        }
    }
    else {
...
/*mexPrintf("alpha(%g), falloff(%g)\n",alpha,falloff);*/
    ...
    pk = mxGetPr(plhs[1]);
    pai = mxGetPr(plhs[2]);
    whirlcone(theta, FOV1, FOV2, FOV3, FOV4, dcf, kmax, NINT, numpt, Ts, Smax_xy, Smax_z, Gmax_xy, Gmax_z, dens_min, pg,
              pk, pai, pn, falloff, alpha, numptx);
